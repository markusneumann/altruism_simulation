...
donation.fit.gain <- 1 #default is 2
donation.fit.loss <- -0.1 #default is -1

#Run the simulation and save the results
source("Neumann_Altruism_Simulation.R")
results_name <- paste0(network_type, "_N", N, "_", mechanism, "_", replication, "_vary_", vary, "_cb_1_10")
...
makeSimulationPlot(results_sims, N, generations, nsim, WS.p.seq, "p (Rewiring probability)", bw = F) + ylab("Percentage of surviving strategies") + ylim(0, 18)
...
# makeSimulationPlot(results_sims, N, generations, nsim, WS.p.seq, "p (Rewiring probability)", bw = T) + ylab("Percentage of surviving strategies") + ylim(0, 18)
